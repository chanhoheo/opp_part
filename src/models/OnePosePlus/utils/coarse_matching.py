...
class CoarseMatching(nn.Module):
    def __init__(self, config, profiler=None):
        ...
        elif self.type == "part-softmax":
            self.temperature = config['part_softmax']['temperature']
        else:
    ...
    def forward(self, feat_db_3d, feat_query, data, mask_query=None):
        ...
        if self.type == "dual-softmax":
            ...
            batch = data['part_weight'].shape[0]
            num_part = data['part_weight'].shape[1]
            ...
            #         sim_matrix[b][data['part_kp_mask'][b][p], :] = sim_matrix[b][data['part_kp_mask'][b][p], :] * data['part_weight'][b][p]

            argmax = torch.argmax(data['part_weight'], dim=1)
            for b, part in enumerate(argmax):
                sim_matrix[b, ~data['part_kp_mask'][b, part, :], :] = 0
            ...
            #     for side in range(len(p)):
            #         cmatrix = F.softmax(sim_matrix[batch, p[side], :], 0) * F.softmax(sim_matrix[batch, p[side], :], 1)
            #         cscore = torch.mean(cmatrix)
            #         score_l.append(cscore)

            #     _score_l = torch.tensor(score_l, device=feat_db_3d.device)
            #     score.append(score_l)
            #     best_part.append(_score_l.argmax())

            # best_part = torch.tensor(best_part, device=feat_db_3d.device)        
            # score = torch.tensor(score, device=feat_db_3d.device)
            # data.update({"part_score": score})
            
            # for batch in range(N):
            #     best = part[batch][best_part[batch]]
            #     mask = torch.zeros_like(sim_matrix[0], dtype=torch.bool)
            #     mask[best,:] = True
            #     sim_matrix[batch, ~mask] = 0
