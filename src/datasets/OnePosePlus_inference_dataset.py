...
class OnePosePlusInferenceDataset(Dataset):
    ...
    def seperate_pc(self, data):
        ...
        p = o3d.utility.Vector3dVector(keypoints3d[0].cpu())
        pcd.points = p
        ...
        for vp in view_point:
            ...
            mask[p_i] = True
            # pad to 7000
            kp = torch.nn.functional.pad(keypoints3d[0][mask], (0, 0, 0, num_point-len(p_i)))
            part_kp_mask.append(mask)
            part_kp_xyz.append(kp)
        
        part_kp_mask = torch.cat(part_kp_mask).view(num_part, -1).unsqueeze(dim=0)
        part_kp_xyz = torch.cat(part_kp_xyz).view(num_part, num_point, -1).unsqueeze(dim=0)
