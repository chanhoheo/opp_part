...
from src.models.PointNet2 import PointNet2_model
...
class OnePosePlus_model(nn.Module):
    def __init__(self, config, profiler=None, debug=False):
        ...
        self.pn = PointNet2_model()


    def encode_part(self, part_desc3d_db, data):
        # part_desc3d_db: [batch, 5, 7000, 256]
        part_score, part_weight = self.pn(part_desc3d_db, data['part_kp_xyz'])  

        return part_score, part_weight # [batch, num_part], [batch, num_part]
    ...
    def forward(self, data):
        ...
        # part
        _desc3d_db = desc3d_db.detach()
        
        num_part = data['part_kp_mask'].shape[1]
        part_desc3d_db = _desc3d_db.unsqueeze(dim=1)
        part_desc3d_db = part_desc3d_db.repeat(1,num_part,1,1) # [batch, num_part, 1, 1]
        part_desc3d_db[data['part_kp_mask']] = 0
        
        # pn2
        part_score, part_weight = self.encode_part(part_desc3d_db, data)
