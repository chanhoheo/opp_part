...
class Loss(nn.Module):
    ...
    def compute_part_loss(self, conf_matrix_gt, part_kp_mask, part_score):
        # conf_matrix_gt: [batch, 7000, 4096 ]
        # part_kp_mask: [batch, part class, 7000]
        # part_score: [batch, part class] 

        b = part_score.shape[0]
        criterion = nn.CrossEntropyLoss()   
        
        #pred [batch, part class]
        #target [batch]
        target =[]
        for batch in range(b):
            t = 0
            m = 0
            for p in range(part_score.shape[1]):
                tmp = conf_matrix_gt[batch, part_kp_mask[batch][p], :].sum()
                if m < tmp:
                    m = tmp
                    t = p
            target.append(t)

        mask = torch.zeros_like(conf_matrix_gt, dtype=torch.bool)
        for b, part_i in enumerate(target):
            mask[b, part_kp_mask[b][part_i], :] = True

        conf_matrix_gt[~mask] = 0
        target = torch.tensor(target, device = part_score.device)

        loss = criterion(part_score, target)    
    ...
    def forward(self, data):
        ...
        # 3. part-level loss
        loss_p = self.compute_part_loss(data['conf_matrix_gt'], data['part_kp_mask'], data['part_score'])
        loss = loss_p * 1.0 #self.config["part_weight"]
