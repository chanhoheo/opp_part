...
class OnePosePlusDataset(Dataset):
    ...
    def seperate_pc(self, data):
        keypoints3d = data['keypoints3d']
        num_part = 5
        num_point = keypoints3d.shape[0]

        # batch = keypoints3d.shape[0]
        view_point = [
                [-1, 0, 0],
                [1, 0, 0],
                [0, 0, 1],
        ...
        p = o3d.utility.Vector3dVector(keypoints3d)
        pcd.points = p
        diameter = np.linalg.norm(np.asarray(pcd.get_max_bound()) - np.asarray(pcd.get_min_bound()))

        for vp in view_point:
            _, p_i = pcd.hidden_point_removal(vp, diameter*5000) 
            mask = torch.zeros((num_point), dtype=torch.bool)
            mask[p_i] = True
            # pad to 7000
            kp = torch.nn.functional.pad(keypoints3d[mask], (0, 0, 0, num_point-len(p_i)))
            part_kp_mask.append(mask)
            part_kp_xyz.append(kp)
        
        part_kp_mask = torch.cat(part_kp_mask).view(num_part, -1)
        part_kp_xyz = torch.cat(part_kp_xyz).view(num_part, num_point, -1)

        return part_kp_mask, part_kp_xyz
    ...
    def read_anno(self, img_id, image_warp_adapt=False):
        ...
        data.update(
            {
                ...
                "descriptors3d_db": avg_descriptors3d,  # [dim, n2] [128, 7000]
        ...
        if avg_coarse_descriptors3d is not None:
            data.update(
                {
                    "descriptors3d_coarse_db": avg_coarse_descriptors3d,  # [dim, n2] [256, 7000]
                }
            )

        part_kp_mask, part_kp_xyz = self.seperate_pc(data)
        data.update(
            {
                "part_kp_mask": part_kp_mask,
                "part_kp_xyz": part_kp_xyz
            }
        )
